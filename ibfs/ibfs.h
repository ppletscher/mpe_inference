...
private:
	...
	struct node
	{
		...
		union
		{
			tcaptype	terminalCap;	// terminalCap > 0 => capacity from the source
										// terminalCap < 0 => -capacity to the sink
			node		*nextOrphan;
		};
	};
...
{
	flowtype f = nodes[nodeIndex].terminalCap;
	if (f > 0)
	{
		capacitySource += f;
	}
	...
	nodes[nodeIndex].terminalCap = capacitySource - capacitySink;
}
